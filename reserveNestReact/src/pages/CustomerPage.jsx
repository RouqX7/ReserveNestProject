...
const CustomerPage = () => {
    ...
    const loadCustomers = async () => {
        try {
            const response = await axios.get("http://localhost:8081/api/v1/customer/getAll");
    ...
    const updateCustomer = async (customer) => {
        try {
            await axios.put(`http://localhost:8081/api/v1/customer/edit/${customer._id}`, customer);
    ...
    return (
        <div>
            ...
            <CustomerForm onSave={editingCustomer ? updateCustomer : saveCustomer} customerData={editingCustomer} />
            <CustomerList customers={customers} onEdit={setEditingCustomer} onDelete={deleteCustomer} />
        </div>
    );
};
